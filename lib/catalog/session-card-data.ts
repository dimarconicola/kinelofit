import { resolveSessionCardDataFromSnapshot, type ResolvedSessionCardData } from '@/lib/catalog/session-card-data.shared';
import type { Session } from '@/lib/catalog/types';

export { resolveSessionCardDataFromSnapshot, type ResolvedSessionCardData };

export const resolveSessionCardData = async (sessions: Session[]) => {
  const { getBookingTarget, getInstructor, getStyle, getVenue } = await import('@/lib/catalog/server-data');
